...
iScroll.prototype = {
	...
	_scrollbar: function (dir) {
		...
		if (!that[dir + 'ScrollbarWrapper']) {
			...
			if (!that.options.scrollbarClass) {
				if(dir == 'v') {
					bar.style.cssText = 'float:right;margin-top:-20px;z-index:100;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.9);-' + vendor + '-background-clip:padding-box;-' + vendor + '-box-sizing:border-box;' + (dir == 'h' ? 'height:100%' : 'width:6px') + ';-' + vendor + '-border-radius:3px;border-radius:3px';
				}
				else{
	...
	refresh: function () {
		var that = this,
			...
			//}, 100);
			//alert(that.maxScrollY + "|" +  that.scrollerH);
			that.dirX = 0;
			that.dirY = 0;
